...
const OrderManagementPage = () => {
  ...
  const handleSelectAll = (
    ...
    if (e.target.checked) {
      ...
      const dateOrders = ordersByDate[date].map((order) => order.id);
      setSelectedOrders(
        selectedOrders.filter((id) => !dateOrders.includes(id))
      );
    }
  };
        ...
        {Object.entries(ordersByDate).map(([date, dateOrders]) => (
          <div key={date}>
            ...
            <div className="grid grid-cols-12 bg-stone-100 p-2 text-sm">
              <div className="col-span-2 flex items-center">
                <input
                  ...
                  checked={dateOrders.every((order) =>
                    selectedOrders.includes(order.id)
                  )}
                />
                <span>{date}</span>
                {date === "12.01.2025" ? (
                  <span className="ml-2 text-gray-500">Заказы за сегодня</span>
                ) : (
                  <span className="ml-2 text-gray-500">Заказы за вечера</span>
                )}
              </div>
            ...
            {orders.map((order) => (
              <div
                key={order.id}
                className="grid grid-cols-12 border-b p-2 text-sm items-center"
              >
                <div className="col-span-1 flex items-center">
                  <input
                    type="checkbox"
